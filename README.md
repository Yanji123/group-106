...
梁芮豪 202100460020 负责project：10 11 18  
潘浩洋 202100460136 负责project：1 2 3 9  

## project1   project2   project3  project9  
这四个实验由小组成员潘浩洋完成，其相关实验过程在word文件“创新创业实践报告”中，在此不做赘述。
...
ECDSA的安全性基于椭圆曲线离散对数问题，这是一种在椭圆曲线上计算的困难问题。该算法涉及椭圆曲线上的点运算和大数运算。以下是ECDSA的主要步骤：  
#### 1. 密钥生成（Key Generation）：
   - 选择一个椭圆曲线，并确定曲线上的基点G。  
   - 随机选择一个私钥（private key）d，它是一个随机的大整数。  
   - 计算公钥（public key）Q = d * G，其中*表示椭圆曲线上的点乘法。  
#### 2. 签名（Signing）：
   - 假设要对消息m进行签名。  
   - 随机选择一个临时私钥k，它是一个随机的大整数。  
   - 计算临时公钥R = k * G。  
   - 计算数字签名s：  
     - 将消息m的哈希值作为整数h。  
     - 计算 s = (k^-1 * (h + d * r)) mod n，其中 k^-1 是 k 的模反元素，r 是 R 的横坐标，n 是椭圆曲线上点的数量。  
#### 3. 验证签名（Verification）：
   - 假设收到消息m、数字签名(s, r)和公钥Q。  
   - 从消息m计算哈希值h。  
   - 计算 w = s^-1 mod n，其中 s^-1 是 s 的模反元素。  
   - 计算 u1 = (h * w) mod n 和 u2 = (r * w) mod n。  
   - 计算点 P = u1 * G + u2 * Q。  
   - 如果 P 的横坐标等于 r，则签名有效；否则，签名无效。  
ECDSA算法具有较高的安全性和较短的密钥长度，因此在资源受限的设备和环境中得到广泛应用，如物联网设备、智能卡、数字证书等场景。它是许多安全通信协议和系统的基础，包括TLS/SSL、SSH、数字证书认证等。
### 运行结果：
